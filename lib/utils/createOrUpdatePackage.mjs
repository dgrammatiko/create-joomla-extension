...
async function updateExistingJSON(currentJSON, type, ans) {
  ...
  currentJSON.joomlaVersion = "4.4.0";
  ...
  switch(type) {
    case 'components':
      currentJSON['joomla-extensions'].components.push({ name: ans.name, version: '0.0.1'});
      break;
    case 'libraries':
      currentJSON['joomla-extensions'].libraries.push({ name: ans.name, version: '0.0.1'});
      break;
    case 'modules':
      currentJSON['joomla-extensions'].modules[ans.client === 'administrator' ? 'administrator' : 'site'].push({ name: ans.name, version: '0.0.1'});
      break;
    case 'templates':
      currentJSON['joomla-extensions'].templates[ans.client === 'administrator' ? 'administrator' : 'site'].push({ name: ans.name, version: '0.0.1'});
      break;
    case 'plugins':
      currentJSON['joomla-extensions'].plugins[ans.folder].push({ name: ans.name, version: '0.0.1'});
      break;
  }
